...
function DriversContent({ onDriversLoaded }) {
  ...
  useEffect(() => {
    const loadDrivers = async () => {
      ...
      try {
        ...
        onDriversLoaded(usersData.length);
      } catch (error) {
    ...
    loadDrivers();
  }, [supabase, statusFilter, onDriversLoaded]);
...
export default function DriversPage() {
  ...
  return (
    <DashboardLayout
      ...
      subtitle={`Total Drivers: ${driverCount}`}
      actions={
    ...
    >
      <Suspense fallback={<DriversLoading />}>
        <DriversContent onDriversLoaded={(count) => setDriverCount(count)} />
      </Suspense>
    </DashboardLayout>
  );
}
